		...
		invasion, metastasis and angiogenesis may be important.</p>
			...
			<div class="row">		
					...
					<% if(zoo_scores.get(level)==null||zoo_scores.get(level)<1){ %>
						<a href="boardgame.jsp?level=<%=level %><%=game_params %>" class="btn btn-primary "><div class="small_level_button"><%=level %></div></a>
						<%}else { %>
						<img width="45" src="images/bc_dog/pink_ribbon_dog_<%=tile_index%>.png">
						<%}%>				
					</div></td>								
